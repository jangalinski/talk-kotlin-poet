 ...
 */
class KotlinTestCompilerLoggingMessageCollector(private val errorSeverity: CompilerMessageSeverity) : MessageCollector {
  ...
  override fun report(severity: CompilerMessageSeverity, message: String, location: CompilerMessageSourceLocation?) {
    ...
    if (location != null) {
      ...
      if (hasExcerpt(location)) {
        stream.print(":\n")
        printExcerpt(location, stream)
      } else {
        stream.print("\n")
      }
    }
  ...
  private fun printExcerpt(location: CompilerMessageSourceLocation, stream: PrintStream) {
    val linesBefore = 3
    val linesAfter = 2
    val linesToPrint = Files.lines(Paths.get(location.path))
      .skip(Math.max(0L, location.line - linesBefore * 1L))
      .limit(linesBefore + linesAfter * 1L)
      .collect(Collectors.toList())
    val tabsInErrorLine = linesToPrint[linesBefore - 1].count { it == '\t' }
    val errorMarkerIndent = location.column + ((TAB_WIDTH - 1) * tabsInErrorLine) - 1;
  ...
  private fun hasExcerpt(location: CompilerMessageSourceLocation) = location.line > -1 && location.column > -1;
}
